/*
 * Copyright (C) 2015 Tokuhiro Matsuno
 * Copyright (C) 2007-2010 Júlio Vilmar Gesser.
 * Copyright (C) 2011, 2013-2015 The JavaParser Team.
 *
 * This file is taken from javaparser.
 * And modified by Tokuhiro Matsuno.
 *
...
public final class JavaFmtVisitor2 implements VoidVisitor<Object> {
	...
	public JavaFmtVisitor() {
		this(true);
	}

	public JavaFmtVisitor(boolean printComments) {
	...
	public void visit(final ClassOrInterfaceDeclaration n, final Object arg) {
		...
		if (!isNullOrEmpty(n.getExtends())) {
			...
			for (final Iterator<ClassOrInterfaceType> i = n.getExtends()
					.iterator(); i.hasNext();) {
	...
	public void visit(final FieldDeclaration n, final Object arg) {
		...
		printer.print(" ");
		for (final Iterator<VariableDeclarator> i = n.getVariables().iterator(); i
				.hasNext();) {
	...
	public void visit(final ConstructorDeclaration n, final Object arg) {
		...
		if (!isNullOrEmpty(n.getThrows())) {
			...
			for (final Iterator<NameExpr> i = n.getThrows().iterator(); i
					.hasNext();) {
	...
	public void visit(final IfStmt n, final Object arg) {
		...
		if (n.getElseStmt() != null) {
			...
			if (elseIf || elseBlock) // put chained if and start of block
										// statement on a same level
				printer.print("else ");
			else {
	...
	public void visit(final ForStmt n, final Object arg) {
		...
		printer.print("for (");
		if (n.getInit() != null) {
			for (final Iterator<Expression> i = n.getInit().iterator(); i
					.hasNext();) {
				final Expression e = i.next();
				e.accept(this, arg);
				if (i.hasNext()) {
		...
		printer.print("; ");
		if (n.getUpdate() != null) {
			for (final Iterator<Expression> i = n.getUpdate().iterator(); i
					.hasNext();) {
				final Expression e = i.next();
				e.accept(this, arg);
				if (i.hasNext()) {
	...
	public void visit(final NormalAnnotationExpr n, final Object arg) {
		...
		printer.print("(");
		if (n.getPairs() != null) {
			for (final Iterator<MemberValuePair> i = n.getPairs().iterator(); i
					.hasNext();) {
	...
	private void printOrphanCommentsEnding(final Node node) {
		...
		for (int i = 0; i < commentsAtEnd; i++) {
			everything.get(everything.size() - commentsAtEnd + i).accept(this,
					null);
		}
	}
}
